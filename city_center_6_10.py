...
##"C:\Python27\ArcGIS10.3\python.exe" "C:\Users\mmiller\Dropbox\Research\Urban\Papers\City Center Resurgence\GIS\Programs\city_center_6_10.py"
...
for c in range(n_s, n_e):
		...
		if c != 9:
			...
			print "- Join to LEHD"
			# Process: Feature Layer
			arcpy.MakeFeatureLayer_management (valid_trcts,  layerName)
			# Process: Add Join
			arcpy.AddJoin_management(layerName, "GEOID", wac, "trct", "KEEP_COMMON")
			arcpy.CopyFeatures_management(layerName, ua_lehd_acs)
			...
			# Process: Add Field
			arcpy.AddField_management(ua_lehd_acs, "EMP_DENS", "DOUBLE", "", "", "", "", "NULLABLE", "NON_REQUIRED", "")

			print "- Compute Emp Dens"
			# Process: Calculate Field
			arcpy.CalculateField_management(ua_lehd_acs, "emp_dens", "[ca01]/ [AREA]", "VB", "")

			print "- Employment Clusters - 500 m"
			# Process: Hot Spot Analysis (Getis-Ord Gi*)
			arcpy.HotSpots_stats(ua_lehd_acs, "emp_dens", uat_emp_hspt, "FIXED_DISTANCE_BAND", "EUCLIDEAN_DISTANCE", "NONE", "500", "", "")
		...
		arcpy.MakeFeatureLayer_management(valid_trcts, urban_area_lyr)
